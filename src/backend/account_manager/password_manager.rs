...
pub async fn get_passwords(jwt: web::Json<JWT>) -> impl Responder {
    let data = PASSWORD_MANAGER.lock().unwrap().get(&jwt.id).unwrap();
    HttpResponse::Ok().json(data.clone())
}
pub async fn add_passwords(jwt: web::Json<JWT>, password: web::Json<Password>) -> impl Responder {
    let data = PASSWORD_MANAGER.lock().unwrap().get_mut(&jwt.id).unwrap();
    data.push(password.clone());
    HttpResponse::Ok().json(data.clone())
}

pub async fn remove_passwords(jwt: web::Json<JWT>, password: web::Json<Password>) -> impl Responder {
    let data = PASSWORD_MANAGER.lock().unwrap().get_mut(&jwt.id).unwrap();
...
pub async fn save_password(jwt: web::Json<JWT>) -> impl Responder {
    ...
    let path = format!("{}{}{}/{}", ROOT.to_str().unwrap(), VAULTS_DATA, jwt.id, PASSWORD);
