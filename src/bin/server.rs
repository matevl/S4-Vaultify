...
use s4_vaultify::backend::account_manager::account_server::{JWTPrivate, UserJson, PRIVATE_DATA, ROOT, USERS_DB};
...
pub async fn create_user_query(user: web::Json<UserJson>) -> impl Responder {
    ...
    HttpResponse::Ok().json(s4_vaultify::backend::account_manager::account_server::JWT::new(new_id, &email))
}
